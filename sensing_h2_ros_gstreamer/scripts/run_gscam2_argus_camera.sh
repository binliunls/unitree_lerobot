...
if [[ $# -lt 6 ]]; then
  echo "Usage: $0 <sensor-id> <node-name> <topic-prefix> <camera-name> <frame-id> <camera-info-yaml> [capture-width] [capture-height] [fps] [output-width] [output-height]" >&2
  exit 2
fi
...
frame_id="$5"
camera_info_yaml="$6"
capture_width="${7:-2560}"
capture_height="${8:-1984}"
fps="${9:-30}"
...
# Argus timestamps buffers; downstream tracing uses these for latency accounting.
export GSCAM_CONFIG="nvarguscamerasrc sensor-id=${sensor_id} ! \
video/x-raw(memory:NVMM),width=${capture_width},height=${capture_height},framerate=${fps}/1 ! \
queue leaky=downstream max-size-buffers=1 max-size-bytes=0 max-size-time=0 ! \
nvvidconv ! video/x-raw,format=NV12,width=${output_width},height=${output_height} ! \
